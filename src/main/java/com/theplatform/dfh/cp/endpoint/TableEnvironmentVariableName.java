package com.theplatform.dfh.cp.endpoint;

public class TableEnvironmentVariableName
{
    public static final String AGENDA = "DB_TABLE_AGENDA";
    public static final String AGENDA_PROGRESS = "DB_TABLE_AGENDA_PROGRESS";
    public static final String OPERATION_PROGRESS = "DB_TABLE_OPERATION_PROGRESS";
    public static final String TRANSFORM = "DB_TABLE_TRANSFORM";
    public static final String CUSTOMER = "DB_TABLE_CUSTOMER";
    public static final String READY_AGENDA = "DB_TABLE_READY_AGENDA";
    public static final String RESOURCE_POOL = "DB_TABLE_RESOURCE_POOL";
    public static final String INSIGHT = "DB_TABLE_INSIGHT";

    private TableEnvironmentVariableName(){}
}
