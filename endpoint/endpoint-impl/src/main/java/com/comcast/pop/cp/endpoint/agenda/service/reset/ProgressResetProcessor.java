package com.comcast.pop.cp.endpoint.agenda.service.reset;
...
import com.comcast.pop.modules.jsonhelper.replacement.JsonReferenceComponents;
    ...
    protected Map<String, OperationDependencies> buildOperationDependencyMap(Agenda agenda)
    {
        ...
        agenda.getOperations().forEach(op ->
        {
            ...
            if(op.getPayload() != null)
            {
                ...
                {
                    Set<String> filteredMissingReferences = filterNonPOPReferences(jsonContext, result.getMissingReferences());
...
//        System.out.println(new JsonHelper().getPrettyJSONString(operationDependencyMap));
     ...
     */
    private Set<String> filterNonPOPReferences(JsonContext jsonContext, Set<String> references)
    {
        ...
        // map all the references that are not pop tokens to operation names (using the JsonReferenceReplacer.extractComponents)
        return references
            .stream()
            .map(ref -> {
